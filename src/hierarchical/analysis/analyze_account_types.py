...
def main():
    ...
    parser.add_argument(
        "--accounts_csv", required=True, help="Path to accounts.csv with 'type' column"
    )
    ...
    parser.add_argument(
        "--use_balance", action="store_true", default=True
    )  # Defaulting to True for Exp14 Prime
    parser.add_argument("--no_counter_party", action="store_true")
    parser.add_argument("--model_type", default="hierarchical")

    parser.add_argument(
        "--max_samples", type=int, default=10000, help="Max accounts to visualize"
    )
    ...
    if umap_res is not None:
        plt.figure(figsize=(10, 8))
        sns.scatterplot(
            x=umap_res[:, 0], y=umap_res[:, 1], hue=labels_valid, alpha=0.6, s=15
        )
