...
def _prepare_inputs(
    df_transactions: pd.DataFrame, df_accounts: pd.DataFrame
    ...
    df_accounts["balanceDateTime"] = pd.to_datetime(
        df_accounts["balanceDateTime"]
    ).dt.date

    # Keep only latest balanceDateTime per account
    df_accounts = df_accounts.sort_values(
        ["accountId", "balanceDateTime"], ascending=[True, False]
    ...
    if "D" in df_transactions["direction"].values:
        ...
        if len(d_vals) > 0:
            ...
            if d_positive_ratio > 0.99:
                ...
                logger.info(
                    "Flipped amount signs (D transactions were positive, now negative)"
                )
    ...
    if "C" in df_transactions["direction"].values:
        ...
        if len(c_vals) > 0:
            ...
            if c_negative_ratio > 0.99:
                df_transactions["amount"] = df_transactions["amount"] * -1
                logger.info(
                    "Flipped amount signs (C transactions were negative, now positive)"
                )
...
def _add_pending_transactions(
    ...
    df_combined = df_combined.sort_values(["accountId", "date", "id"]).reset_index(
        drop=True
    )
...
def _calculate_running_balance(
    ...
    if missing_balance > 0:
        affected = df_result[df_result["availableBalance"].isna()][
            "accountId"
        ].nunique()
        logger.warning(
            f"{affected} accounts have no availableBalance in DAccount. Their balance_after will be NaN."
        )
    ...
    historical_sums = (
        df_result[is_historical].groupby("accountId")["amount"].sum().reset_index()
    )
    ...
    df_result["historical_total_amount"] = df_result["historical_total_amount"].fillna(
        0.0
    )
    ...
    df_result["initial_balance"] = (
        df_result["availableBalance"] - df_result["historical_total_amount"]
    )
    ...
    df_result["balance_after"] = (
        df_result["initial_balance"] + df_result["cumsum_amount"]
    )
...
def calculate_daily_balances(
    ...
    df_detailed = calculate_balance_per_transaction(
        df_transactions, df_accounts, include_pending=include_pending
    )

    # 2. Extract strictly date part
    # Handle NaT or non-date inputs gracefully if possible, but expecting valid inputs
    try:
        if not df_detailed.empty and not isinstance(
            df_detailed["date"].iloc[0], (datetime.date, type(pd.NaT))
        ):
...
class BalanceFeatureExtractor:
    ...
    def __init__(
        ...
        self.daily_balances = calculate_daily_balances(
            df_transactions, df_acc, include_pending=include_pending
        )
        ...
        for acc_id, group in self.daily_balances.groupby("accountId"):
            self._account_data[str(acc_id).upper()] = group.sort_values(
                "date"
            ).reset_index(drop=True)
    ...
    def get_daily_balance(self, account_id: str, date: Any) -> float | None:
    ...
    def get_starting_balance(self, account_id: str, date: Any) -> float | None:
    ...
    def get_window_stats(
        self, account_id: str, start_date: Any, end_date: Any
    ) -> dict[str, float | None]:
