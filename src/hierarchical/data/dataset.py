...
class HierarchicalDataset(Dataset):
    ...
    def __init__(
        ...
        self.feature_extractor = V4FeatureExtractor(
            cat_group_vocab, cat_sub_vocab, counter_party_vocab,
            balance_extractor, epoch_date
        )
        ...
        logger.info(f"HierarchicalDataset: {len(self.accounts)} accounts (min_days={min_days})")

    def __len__(self) -> int:
        return len(self.accounts)

    def _pack_stream(self, df: pd.DataFrame, account_id: str) -> dict[str, torch.Tensor | int | float] | None:
        """Packs a stream of transactions into tensors."""
        n_tx = len(df)
        if n_tx == 0:
            return None

        def get_col(df_: pd.DataFrame, candidates: list[str], default: Any = None) -> Any:
            for c in candidates:
                if c in df_.columns:
                    return df_[c]
        ...
        c_grp = [
            ...
            for x in get_col(df, ['personeticsCategoryGroupId', 'p_categoryGroupId'], 'UNK')
        ]
        c_sub = [
        ...
        if self.feature_extractor.balance_extractor:
            feats = self.feature_extractor.extract_balance_features(account_id, df['date'])
        ...
        return {
            ...
            'balance': torch.tensor(bal_feats, dtype=torch.float32) if bal_feats is not None else None,
            'n_txns': n_tx,
...
def collate_hierarchical(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def init_stream_tensors(T_dim: int) -> dict[str, torch.Tensor | None]:
        ...
        tensors['cat_cp'] = torch.zeros(batch_size, max_days, T_dim, dtype=torch.long) if has_cp else None
        tensors['balance'] = torch.zeros(batch_size, max_days, T_dim, 7, dtype=torch.float32) if has_balance else None
    ...
    for i, item in enumerate(batch):
        ...
        for d, day in enumerate(days):
            ...
            def fill_stream(
                ...
                # Fill tensors
                target_dict['cat_group'][i, d, :n] = as_tensor(source_data['cat_group'], torch.long)
                target_dict['cat_sub'][i, d, :n] = as_tensor(source_data['cat_sub'], torch.long)
                target_dict['amounts'][i, d, :n] = as_tensor(source_data['amounts'], torch.float32)
                target_dict['dates'][i, d, :n] = as_tensor(source_data['dates'], torch.float32)
                
                if target_dict['cat_cp'] is not None and source_data.get('cat_cp') is not None:
                     target_dict['cat_cp'][i, d, :n] = as_tensor(source_data['cat_cp'], torch.long)

                if target_dict['balance'] is not None and source_data.get('balance') is not None:
                    target_dict['balance'][i, d, :n] = as_tensor(source_data['balance'], torch.float32)
                
                if 'log_total_volume' in source_data:
                     # Should be a scalar tensor or float. Force float for numpy scalars.
                     target_dict['log_total_volume'][i, d] = float(source_data['log_total_volume'])
