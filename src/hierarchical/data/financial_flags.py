...
def get_target_subcategories(config: dict) -> list:
    ...
    target_subcategories = [
        subcat_id for subcat_id, info in categories.items()
...
def load_recurring_patterns(file_path: str) -> pd.DataFrame:
    ...
    # Only load columns we need
    df = pd.read_csv(file_path, usecols=['patternId', 'patternTypeBusiness', 'deviceId'], low_memory=False)
...
def filter_transactions(
    ...
    print(f"  Direction: {direction} ({'Debit/Outgoing' if direction == 'D' else 'Credit/Incoming'})")
    ...
    print(f"  Filtered from {initial_count:,} to {len(df):,} transactions ({len(df)/initial_count*100:.1f}%)")
...
def enrich_with_categories(df: pd.DataFrame, categories: dict) -> pd.DataFrame:
    ...
    name_map = {
        subcat_id: info.get('name', subcat_id)
...
def detect_emerging(
    ...
    for subcat_id in target_subcategories:
        ...
        first_txns = subcat_df.loc[first_txn_idx, ['user_name', 'accountId', 'date']].copy()
        ...
        merged['months_since_start'] = merged.apply(
            lambda row: relativedelta(row['emerging_date'], row['account_start_date']).months +
                       relativedelta(row['emerging_date'], row['account_start_date']).years * 12 +
                       relativedelta(row['emerging_date'], row['account_start_date']).days / 30.44,
            axis=1
        )
    ...
    if results:
        ...
        return result_df[['user_name', 'accountId', 'flag_name', 'categoryGroupId', 'category_group_name',
                         'subcategoryId', 'sub_category_name',
                         'emerging_date', 'account_start_date', 'months_since_start']]
    else:
...
def detect_skipped_payment(
    ...
    if monthly_payments_config:
        print(f"  MonthlyPayments filter: {monthly_payments_config['pattern_type']} with {monthly_payments_config['gap_threshold_days']} days threshold")
    print(f"  Bi-monthly exclusion: patterns with 3 txns and all gaps > {bimonthly_gap_threshold} days")

    # Check if patternId column exists
    if 'patternId' not in df.columns:
        print("  Warning: patternId column not found. Skipping skipped payment detection.")
        return pd.DataFrame()
    ...
    if all_pattern_types:
        pattern_df = pattern_df[pattern_df['patternTypeBusiness'].isin(all_pattern_types)]
    ...
    for (user_name, pattern_id), group in pattern_df.groupby(['user_name', 'patternId']):
        ...
        device_id = sorted_group['deviceId'].iloc[0] if 'deviceId' in sorted_group.columns else None
        account_id = sorted_group['accountId'].iloc[0] if 'accountId' in sorted_group.columns else None
        ...
        for i, gap_days in gaps:
            if gap_days >= threshold:
                results.append({
                    ...
                    'deviceId': device_id,
                    'payment_before_gap': pd.Timestamp(dates[i-1]).strftime('%Y-%m-%d'),
                    'payment_after_gap': pd.Timestamp(dates[i]).strftime('%Y-%m-%d'),
                    'gap_days': gap_days
                })
...
def main(config_path: str = "config.yaml"):
    ...
    if recurring_patterns_df is not None:
        skipped_payment_flags = detect_skipped_payment(
            df, recurring_patterns_df, pattern_types, gap_threshold_days, min_transactions, bimonthly_gap_threshold,
