...
def process_single_account(account_id, df, vocabs, balance_cache, config):
    ...
    for day in days:
        ...
        for name, sub_df in [("pos", pos_df), ("neg", neg_df)]:
            ...
            # Encode
            c_grp = [
                cat_group_vocab.encode(x)
                for x in get_col(
                    sub_df, ["personeticsCategoryGroupId", "p_categoryGroupId"]
                )
            ]
            c_sub = [
                cat_sub_vocab.encode(x)
                for x in get_col(
                    sub_df, ["personeticsSubCategoryId", "p_subCategoryId"]
                )
            ]
            c_cp = [
                counter_party_vocab.encode(x)
                for x in get_col(sub_df, ["deviceId", "counter_party"])
            ]
            ...
            norm_amounts = (log_amounts - log_amounts.mean()) / (
                log_amounts.std() + 1e-9
            )
...
def process_account_chunk(chunk_data):
    ...
    for account_id, df in chunk_data:
        results.append(
            process_single_account(account_id, df, _VOCABS, _BALANCE_CACHE, _CONFIG)
        )
    return results
...
def precompute_balance_cache(df: pd.DataFrame, balance_extractor) -> dict:
    ...
    cache = {}
    forCtx = tqdm(
        unique_pairs.iterrows(), total=len(unique_pairs), desc="Balance Cache"
    )
    for _, row in forCtx:
        ...
        try:
            ...
            starting_bal = (
                balance_extractor.get_starting_balance(acc_id, date_val) or 0.0
            )
            ...
            feats = np.array(
                [
                    ...
                    balance_extractor.normalize_balance(
                        stats.get("balance_volatility")
                    ),
...
def main():
    ...
    parser.add_argument(
        "--model_checkpoint",
        required=False,
        help="Path to existing model (to reuse vocab)",
    )
    parser.add_argument(
        "--vocab_dir", required=False, help="Directory to load/save vocabularies"
    )
    ...
    if args.model_checkpoint:
        ...
        try:
            ...
            if not args.data_file:
                raise ValueError("--data_file required to build vocabs")
    ...
    cache_source = (
        ...
        if args.mode == "train"
            ...
            Path(args.eval_data_dir).parent / "pretrain" / "pretrain_transactions.csv"
        )
    )
    ...
    if os.path.exists(cache_source):
        ...
        logger.warning(
            f"Cache source {cache_source} not found. Balance features may be zero."
        )
        balance_cache = {}
    ...
    def process_dataframe(df, output_path):
        ...
        if args.censor_subcat:
            ...
            if col_name:
                ...
                series_str = (
                    df[col_name].astype(str).str.upper().str.lstrip("C").str.lstrip("G")
                )
            ...
            logger.info(
                f"  CENSORED {drop_len} transactions matching {args.censor_subcat} (Norm: {target})"
            )
        ...
        logger.info(
            f"  {len(groups)} accounts, {len(chunks)} chunks. Using {cpu_count()} cores."
        )
        ...
        with Pool(
            ...
            initargs=(vocabs, balance_cache, config),
        ) as pool:
            for res in tqdm(
                pool.imap(process_account_chunk, chunks), total=len(chunks)
            ):
    ...
    if args.mode == "train":
        if not args.data_file:
            raise ValueError("--data_file required for train mode")
        ...
        if not args.eval_data_dir:
            raise ValueError("--eval_data_dir required for eval mode")
