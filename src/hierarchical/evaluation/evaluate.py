...
def load_model(
    checkpoint_path, device, hidden_dim=128, model_type="hierarchical", args=None
):
    ...
    if isinstance(checkpoint, dict) and "config" in checkpoint:
        ...
        if use_amount_binning:
            num_amount_bins = state_dict[
                "day_encoder.txn_encoder.amount_emb.weight"
            ].shape[0]
        ...
        cat_grp_size = state_dict.get(
            f"{prefix}cat_group_emb.weight", torch.zeros(100, 1)
        ).shape[0]
        cat_sub_size = state_dict.get(
            f"{prefix}cat_sub_emb.weight", torch.zeros(100, 1)
        ).shape[0]
        cat_cp_size = state_dict.get(
            f"{prefix}counter_party_emb.weight", torch.zeros(100, 1)
        ).shape[0]
        ...
        d_txn = (
            args.txn_dim if hasattr(args, "txn_dim") and args.txn_dim else hidden_dim
        )
        d_day = (
            args.day_dim if hasattr(args, "day_dim") and args.day_dim else hidden_dim
        )
        d_acc = (
            args.account_dim
            if hasattr(args, "account_dim") and args.account_dim
            else hidden_dim
        )
        ...
        logger.info(
            f"  Inferred from state_dict: cat_grp={cat_grp_size}, cat_sub={cat_sub_size}, cat_cp={cat_cp_size}"
        )
            ...
            for stream_key in ["pos", "neg"]:
                if stream_key in day and day[stream_key] is not None:
                    ...
                    if "balance" in stream:
                        ...
                        if hasattr(bal_data, "cpu"):
                            bal_data = bal_data.cpu().numpy()
                        if len(bal_data) > 0:
...
def main():
    ...
    parser.add_argument(
        "--tensors_path", required=True, help="Path to precomputed tensors .pt file"
    )
    parser.add_argument(
        "--emerging_csv", required=True, help="Path to emerging flags CSV"
    )
    parser.add_argument(
        "--model_checkpoint", required=True, help="Path to model checkpoint"
    )
    ...
    parser.add_argument(
        "--T_values", type=str, default="1,3,7", help="Comma-separated T values"
    )
    parser.add_argument(
        "--model_type",
        choices=["hierarchical", "day_ae"],
        default="hierarchical",
        help="Model architecture type",
    )
    parser.add_argument(
        "--use_censored_positives",
        action="store_true",
        help="Use censored positives logic (TODO)",
    )
    parser.add_argument(
        "--csv_output_metrics",
        default="ensemble_summary_metrics.csv",
        help="Output CSV name for metrics",
    )
    parser.add_argument(
        "--csv_output_curves",
        default="cumulative_lift_curves.csv",
        help="Output CSV name for curves",
    )
    parser.add_argument(
        "--seeds",
        type=str,
        default="42",
        help="Comma-separated random seeds for train/test split (e.g. 42,123,456)",
    )
    parser.add_argument(
        "--negatives_in_training",
        type=str,
        default="1000,2000,5000,10000,-1",
        help="Comma-separated list of max train negatives values to loop over. -1 for no limit.",
    )
    ...
    emerging_df["global_id"] = (
        emerging_df["bank"] + "_" + emerging_df["accountId"].astype(str)
    )
    ...
    model = load_model(
        args.model_checkpoint, device, args.hidden_dim, args.model_type, args
    )
    ...
    for T in T_values:
        ...
        for acc_id in tqdm(all_account_ids, desc=f"Global Cache (T={T})"):
            ...
            negative_cache_hard[acc_id][T] = {
                "feat": extract_tensor_features(item_sliced)
            }

        logger.info(
            f"  T={T}: Skipped {skipped_span} (span<{MIN_CALENDAR_SPAN}d), {skipped_active} (active<{MIN_ACTIVE_DAYS})"
        )
    ...
    for seed in seeds:
        ...
        train_accounts, test_accounts = train_test_split(
            all_account_ids, test_size=args.test_size, random_state=seed
        )
        ...
        for neg_train_val in neg_train_values:
            ...
            for flag in tqdm(
                all_flags, desc=f"Evaluating (Seed {seed}, negs={neg_train_val})"
            ):
                flag_positive_ids = set(
                    row["global_id"]
                    for _, row in emerging_df[
                        emerging_df["flag_name"] == flag
                    ].iterrows()
                )
                ...
                for T in T_values:
                    ...
                    for acc_id in flag_positive_ids:
                        if acc_id not in train_ids_set:
                            continue  # Must be in train split
                    ...
                    for acc_id in train_negative_ids:
                        ...
                        if acc_id in flag_positive_ids:
                            continue

                        if (
                            acc_id in negative_cache_hard
                            and T in negative_cache_hard[acc_id]
                        ):
                    ...
                    # 5. Linear
                    clf_lin_emb = LogisticRegression(
                    ...
                    if HAS_CLEANLAB:
                        try:
                            base_lr_emb = LogisticRegression(
                                max_iter=1000,
                            ...
                            base_lr_feat = LogisticRegression(
                                max_iter=1000,
                                class_weight="balanced",
                                random_state=seed,
                                n_jobs=-1,
                    ...
                    for acc_id in test_accounts:
                        ...
                        if is_pos:
                            ...
                            if (
                                ...
                                X_test_feat.append(
                                    negative_cache_hard[acc_id][T]["feat"]
                                )
                                y_test.append(0)
                    ...
                    def evaluate_method(method_name, clf, X_emb, X_feat):
                        ...
                        try:
                            ...
                            auc = 0.5
                        lifts, confs, recalls, precisions, f1s, n_founds, depths = (
                            compute_lift_curve(y_test, probs)
                        )
                    ...
                    for m_name, clf in methods:
                        if clf is None:
                            continue
                        if m_name == "XGB_Bal" and not has_bal:
                            continue
    ...
    if not all_results:
        logger.warning(
            "No results generated! Check intersections of flags and tensors."
        )
        return
    ...
    logger.info("=" * 80)
    logger.info(
        f"{'Flag':<30} {'T':>2} {'Method':<10} {'AUC':>6} {'L@5':>7} {'L@10':>7}"
    )
    logger.info("-" * 80)

    # Aggregate for display
    summary_df = (
        results_df.groupby(["flag", "T", "method"])[["auc", "lift_5", "lift_10"]]
        .mean()
        .reset_index()
    )
    ...
    for _, r in summary_df.sort_values(["flag", "T", "method"]).iterrows():
        ...
        l10 = r["lift_10"]
        logger.info(
            f"{r['flag']:<30} {r['T']:>2} {r['method']:<10} {auc:>6.3f} {l5:>6.2f}x {l10:>6.2f}x"
        )
