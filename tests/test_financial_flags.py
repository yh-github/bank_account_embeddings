...
class TestFinancialFlags(unittest.TestCase):
    def test_generate_flag_name(self):
        self.assertEqual(generate_flag_name("Buy Now Pay Later", "EMERGING"), "EMERGING_BUY_NOW_PAY_LATER")
        self.assertEqual(generate_flag_name("Savings & Investment", "EMERGING"), "EMERGING_SAVINGS_INVESTMENT")
        self.assertEqual(generate_flag_name("  Bad   Spacing  ", "EMERGING"), "EMERGING_BAD_SPACING")
    ...
    def test_detect_skipped_payment(self):
        ...
        dates_skip = [datetime(2023, 1, 1), datetime(2023, 3, 1), datetime(2023, 4, 1), datetime(2023, 5, 1)]
        
        data = []
        # Regular
        for d in dates_reg:
            data.append({'user_name': 'U1', 'accountId': 'A1', 'date': d, 'patternId': 'P1'})
        # Skip
        for d in dates_skip:
            data.append({'user_name': 'U2', 'accountId': 'A2', 'date': d, 'patternId': 'P2'})
    ...
    def test_skipped_payment_exclude_bimonthly(self):
        ...
        flags = detect_skipped_payment(
            df, recurring_patterns, ['DateOfMonth'], 45, min_transactions=3, bimonthly_gap_threshold=50
        )
        self.assertTrue(flags.empty)
