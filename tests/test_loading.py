...
class TestModelLoading(unittest.TestCase):
    ...
    def test_day_encoder_uses_layers_module_list(self):
        ...
        self.assertTrue(
            hasattr(day_enc, "layers"), "DayEncoder should have 'layers' attribute"
        )
        self.assertEqual(len(day_enc.layers), 2, "DayEncoder should have 2 layers")

        # Verify it doesn't have self.transformer
        self.assertFalse(
            hasattr(day_enc, "transformer"),
            "DayEncoder should NOT have 'transformer' attribute",
        )
    ...
    def test_account_encoder_uses_transformer(self):
        ...
        self.assertTrue(
            ...
            "AccountEncoder should have 'transformer' attribute",
        )
    ...
    def test_state_dict_key_structure(self):
        ...
        self.assertGreater(
            len(day_layer_keys), 0, "Should have day_encoder.layers keys"
        )

        # Check for AccountEncoder.transformer keys (TransformerEncoder)
        acc_transformer_keys = [
            k for k in sd.keys() if k.startswith("transformer.layers.")
        ]
        self.assertGreater(
            len(acc_transformer_keys), 0, "Should have transformer.layers keys"
        )
        ...
        self.assertEqual(
            len(old_day_keys),
            0,
            "Should NOT have day_encoder.transformer keys (old architecture)",
        )
