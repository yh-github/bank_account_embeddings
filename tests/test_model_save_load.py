    ...
    def test_save_load_with_config(self):
        ...
        try:
            ...
            def make_stream(B, D, T, config):
                return {
                    "cat_group": torch.randint(
                        0, config["num_categories_group"], (B, D, T)
                    ),
                    "cat_sub": torch.randint(
                        0, config["num_categories_sub"], (B, D, T)
                    ),
                    "cat_cp": torch.randint(
                        0, config["num_counter_parties"], (B, D, T)
                    ),
                    "amounts": torch.randn(B, D, T),
                    "dates": torch.zeros(B, D, T, 4),
